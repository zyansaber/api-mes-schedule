...
const GREENRV_API_KEY = process.env.GREENRV_SCHEDULING_API_KEY || "";
...
const authorizeGreenRvRequest = (req, res) => {
  if (!GREENRV_API_KEY || !GREENRV_ENCRYPTION_KEY) {
    res.status(503).json({
  ...
  if (!constantTimeEquals(suppliedKey, GREENRV_API_KEY)) {
    res.status(401).json({
      error: "Unauthorized",
