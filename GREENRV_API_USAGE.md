...
The API reads from the Firebase `schedule` node and only includes orders whose `Dealer` is one of:
...
The response does not include `Vin Number` or `production status`.
